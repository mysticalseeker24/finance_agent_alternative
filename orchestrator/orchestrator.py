...
class Orchestrator:
    ...
    async def initialize_agents(self) -> Dict[str, str]:
        ...
        initialization_tasks = [
            self._initialize_agent("retriever_agent", self.retriever_agent.initialize()),
            self._initialize_agent("language_agent", self.language_agent.initialize_llm()),
            self._initialize_agent("voice_agent", self.voice_agent.initialize_stt_model()),
        ]
    ...
    async def _initialize_agent(self, agent_name: str, init_coroutine) -> None:
        ...
        try:
            ...
            logger.info(f"Agent {agent_name} initialization: {self.agent_status[agent_name]}")
        except Exception as e:
            self.agent_status[agent_name] = "error"
            logger.error(f"Error initializing {agent_name}: {str(e)}")
    
    async def process_query(self, query: str, voice_input: bool = False, voice_output: bool = True) -> Dict[str, Any]:
        ...
        try:
            ...
            if voice_input:
                ...
                if "error" in stt_response:
                    raise HTTPException(status_code=400, detail=f"Speech-to-text error: {stt_response['error']}")
            ...
            if "error" in response_result:
                raise HTTPException(status_code=500, detail=f"Response generation error: {response_result['error']}")
    ...
    async def generate_market_brief(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if "error" in brief_response:
                raise HTTPException(status_code=500, detail=f"Brief generation error: {brief_response['error']}")
            ...
            result = {
                ...
                "sections": {
                    ...
                    "portfolio_performance": brief_data.get("portfolio_performance", ""),
    ...
    async def _retrieve_context(self, query: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if not results or max([r.get("score", 0) for r in results] or [0]) < 0.7:
                logger.info("Low quality retrieval results, trying to reformulate query")
    ...
    async def _gather_data_for_query(self, query: str, context: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        if any(term in query_lower for term in ["portfolio", "holdings", "positions", "stocks", "investment"]):
            # This would typically come from a database or user configuration
            # For this example, we'll use a placeholder portfolio
            portfolio = [
                {"ticker": "AAPL", "weight": 0.15},
        ...
        if any(term in query_lower for term in ["market", "index", "s&p", "dow", "nasdaq"]):
            market_response = await self.api_agent.get_market_summary()
            gathered_data["market_data"] = market_response.get("data", {})
        
        # Check for news-related queries
        if any(term in query_lower for term in ["news", "headline", "announcement", "report"]):
            news_response = await self.scraping_agent.scrape_market_news()
        ...
        if any(term in query_lower for term in ["risk", "exposure", "volatility", "beta"]):
            ...
            if "portfolio_data" in gathered_data:
                risk_response = await self.analysis_agent.calculate_risk(tickers=[item["ticker"] for item in portfolio])
    ...
    def _extract_stock_tickers(self, query: str) -> List[str]:
        ...
        common_tickers = {
            "AAPL", "MSFT", "AMZN", "GOOGL", "BRK.B", "JNJ", "JPM", "V", "PG", "UNH",
            "TSMC", "ASML", "NVDA", "TSM", "INTC", "AMD", "FB", "META", "NFLX", "BABA"
        }
        
        words = query.upper().split()
        found_tickers = [word.strip(",.?!()") for word in words if word.strip(",.?!()") in common_tickers]
