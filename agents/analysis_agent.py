...
class AnalysisAgent(BaseAgent):
    ...
    async def process(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if operation == "calculate_risk":
            ...
            benchmark_historical_prices_df = parameters.get(
                "benchmark_historical_prices"
            )
            risk_free_rate_param = parameters.get("risk_free_rate", 0.0)

            if asset_historical_prices_df is None or not isinstance(
                asset_historical_prices_df, pd.DataFrame
            ):
                return {
                    "error": "Asset historical prices (DataFrame) must be provided for risk calculation"
                }
            if benchmark_historical_prices_df is not None and not isinstance(
                benchmark_historical_prices_df, pd.DataFrame
            ):
                return {
                    "error": "Benchmark historical prices must be a DataFrame if provided"
                }
            ...
            portfolio_composition = parameters.get(
                "portfolio"
            )  # The list of {"ticker": "X", "weight": Y}
            asset_metadata_dict = parameters.get(
                "asset_metadata"
            )  # The dict of {"TICKER": stock_info_dict}
            portfolio_asset_historical_prices_df = parameters.get(
                "portfolio_asset_historical_prices"
            )  # DataFrame of prices
            benchmark_historical_prices_df = parameters.get(
                "benchmark_historical_prices"
            )  # Optional DataFrame
            ...
            if not portfolio_composition:
                return {
                    "error": "Portfolio composition list must be provided for analysis"
                }
            if portfolio_asset_historical_prices_df is None or not isinstance(
                portfolio_asset_historical_prices_df, pd.DataFrame
            ):
                return {
                    "error": "Portfolio asset historical prices (DataFrame) must be provided"
                }
            if benchmark_historical_prices_df is not None and not isinstance(
                benchmark_historical_prices_df, pd.DataFrame
            ):
            ...
            if asset_historical_prices_df is None or not isinstance(
                ...
                return {
                    "error": "Asset historical prices (DataFrame) must be provided for performance analysis"
                }
            if benchmark_historical_prices_df is not None and not isinstance(
                benchmark_historical_prices_df, pd.DataFrame
            ):
            ...
            if asset_historical_prices_df is None or not isinstance(
                ...
                return {
                    "error": "Asset historical prices (DataFrame) must be provided for anomaly detection"
                }

            result = await self._detect_anomalies(
                asset_historical_prices_df, threshold_param
            )
            return {"data": result}

        elif operation == "get_earnings_surprises":
            tickers = parameters.get("tickers")
            api_agent_instance = parameters.get(
                "api_agent"
            )  # Orchestrator must pass this

            if not tickers:
                return {"error": "No tickers provided for get_earnings_surprises"}
            if not api_agent_instance:
                return {
                    "error": "APIAgent instance not provided for get_earnings_surprises"
                }

            result = await self.get_earnings_surprises(tickers, api_agent_instance)
            return {
                "data": result
            }  # The method already returns data in the desired structure
    ...
    async def _calculate_risk(
        ...
        try:
            if (
                not isinstance(asset_historical_prices, pd.DataFrame)
                or asset_historical_prices.empty
            ):
                return {
                    "error": "Asset historical prices must be a non-empty DataFrame."
                }
            if (
                "Date" not in asset_historical_prices.columns
                or "Close" not in asset_historical_prices.columns
            ):
                return {
                    "error": "Asset historical prices DataFrame must contain 'Date' and 'Close' columns."
                }
            ...
            if asset_returns.empty:
                return {
                    "error": "Asset returns could not be calculated (e.g., single data point)."
                }

            # Volatility (Annualized)
            volatility = asset_returns.std() * np.sqrt(252)
            if pd.isna(volatility):
                volatility = 0.0  # Handle case with single return value (std is NaN)
            ...
            if (
                benchmark_historical_prices is not None
                and not benchmark_historical_prices.empty
            ):
                if (
                    "Date" not in benchmark_historical_prices.columns
                    or "Close" not in benchmark_historical_prices.columns
                ):
                    logger.warning(
                        "Benchmark historical prices DataFrame is missing 'Date' or 'Close' columns. Skipping Beta calculation."
                    )
                else:
                    ...
                    if not benchmark_returns.empty:
                        ...
                        if (
                            len(aligned_df) > 1
                        ):  # Need at least 2 data points for covariance
                            aligned_asset_returns = aligned_df["asset"]
                            aligned_benchmark_returns = aligned_df["benchmark"]

                            covariance_matrix = np.cov(
                                aligned_asset_returns, aligned_benchmark_returns
                            )
                            covariance_with_benchmark = covariance_matrix[0, 1]
                            variance_benchmark = np.var(
                                aligned_benchmark_returns
                            )  # Using numpy's var for consistency

                            if variance_benchmark == 0:
                                beta = (
                                    np.nan
                                )  # Or some other indicator of undefined Beta
                            else:
                                beta = covariance_with_benchmark / variance_benchmark
                        else:
                            logger.warning(
                                "Not enough overlapping data points between asset and benchmark to calculate Beta."
                            )
                    else:
                        logger.warning(
                            "Benchmark returns could not be calculated. Skipping Beta calculation."
                        )
    ...
    async def _analyze_portfolio(
        ...
        try:
            ...
            if not portfolio or not isinstance(portfolio, list):
                return {
                    "error": "Invalid portfolio structure for composition analysis."
                }
            ...
            if not tickers:
                logger.warning(
                    "Portfolio composition analysis requested for an empty portfolio."
                )
                ...
                if abs(weight_sum - 1.0) > 1e-6 and weight_sum != 0:
                    logger.info(
                        f"Normalizing portfolio weights for composition. Original sum: {weight_sum}"
                    )
                    weights_list = [w / weight_sum for w in weights_list]
                elif weight_sum == 0:
                    logger.warning(
                        "Portfolio weights sum to zero for composition. Distributing weights equally."
                    )
                ...
                for ticker, weight in zip(tickers, weights_list):
                    ...
                    sector = (
                        stock_info.get("sector", "Unknown") if stock_info else "Unknown"
                    )
                    country = (
                        stock_info.get("country", "Unknown")
                        if stock_info
                        else "Unknown"
                    )
                    ...
                    if quote_type == "EQUITY":
                        ...
                        asset_type = "Equity" if stock_info else "Unknown"
                    if not stock_info:
                        logger.warning(
                            f"No asset metadata for {ticker} in composition analysis."
                        )

                    sector_exposure[sector] = sector_exposure.get(sector, 0) + weight
                    country_exposure[country] = (
                        country_exposure.get(country, 0) + weight
                    )
                ...
                composition_result = {
                    ...
                    "country_exposure": [
                        {"country": c, "weight": w} for c, w in country_exposure.items()
                    ],
                    "asset_type_exposure": [
                        {"asset_type": at, "weight": w}
                        for at, w in asset_type_exposure.items()
                    ],
            ...
            if (
                not isinstance(portfolio_asset_historical_prices, pd.DataFrame)
                or portfolio_asset_historical_prices.empty
            ):
                logger.warning(
                    "Portfolio historical prices DataFrame is missing or empty. Skipping portfolio risk metric calculations."
                )
                composition_result["risk_metrics"] = {
                    "error": "Historical price data for portfolio assets not provided or empty."
                }
                return composition_result  # Return composition even if risk cannot be calculated

            # Ensure 'Date' index is datetime
            if not isinstance(
                portfolio_asset_historical_prices.index, pd.DatetimeIndex
            ):
                try:  # If Date is a column
                    portfolio_asset_historical_prices["Date"] = pd.to_datetime(
                        portfolio_asset_historical_prices["Date"]
                    )
                    portfolio_asset_historical_prices = (
                        portfolio_asset_historical_prices.set_index("Date")
                    )
                except KeyError:  # Date column does not exist
                    return {
                        "error": "Portfolio historical prices DataFrame must have a 'Date' index or 'Date' column."
                    }

            portfolio_asset_historical_prices = (
                portfolio_asset_historical_prices.sort_index()
            )
            asset_returns_df = portfolio_asset_historical_prices.pct_change().dropna()

            if asset_returns_df.empty:
                logger.warning(
                    "Asset returns DataFrame is empty after processing. Skipping portfolio risk metrics."
                )
                composition_result["risk_metrics"] = {
                    "error": "Could not calculate asset returns for portfolio risk analysis."
                }
            ...
            portfolio_tickers_set = set(
                tickers
            )  # Tickers from the portfolio composition
            ...
            for ticker_col in asset_returns_df.columns:
                if ticker_col in portfolio_tickers_set:
                    try:
                        ...
                        logger.error(
                            f"Ticker {ticker_col} from price data not found in portfolio composition tickers. This is unexpected."
                        )
                    ...
                    logger.warning(
                        f"Ticker {ticker_col} in historical prices not in defined portfolio. Assigning zero weight for risk calculation."
                    )
                    aligned_weights_list.append(0)

            if not valid_tickers_for_returns or sum(aligned_weights_list) == 0:
                logger.warning(
                    "No valid tickers with weights found matching historical price data columns. Cannot calculate portfolio returns."
                )
                composition_result["risk_metrics"] = {
                    "error": "No matching tickers between portfolio definition and historical price data columns for risk calculation."
                }
                return composition_result
            ...
            if (
                abs(current_aligned_weight_sum - 1.0) > 1e-6
                and current_aligned_weight_sum != 0
            ):
                logger.info(
                    f"Normalizing aligned weights for risk calculation. Original sum: {current_aligned_weight_sum}"
                )
                aligned_weights_list = [
                    w / current_aligned_weight_sum for w in aligned_weights_list
                ]

            weights_series = pd.Series(
                aligned_weights_list, index=valid_tickers_for_returns
            )
            ...
            portfolio_daily_returns = (filtered_asset_returns_df * weights_series).sum(
                axis=1
            )

            if portfolio_daily_returns.empty:
                logger.warning(
                    "Portfolio daily returns are empty. Skipping portfolio risk metrics."
                )
                composition_result["risk_metrics"] = {
                    "error": "Portfolio daily returns could not be calculated."
                }
                return composition_result
            ...
            portfolio_drawdown = (
                cumulative_portfolio_returns - portfolio_peak
            ) / portfolio_peak
            portfolio_max_drawdown = portfolio_drawdown.min()
            if pd.isna(portfolio_max_drawdown):
            ...
            if (
                ...
                if not isinstance(benchmark_historical_prices.index, pd.DatetimeIndex):
                    try:
                        benchmark_historical_prices["Date"] = pd.to_datetime(
                            benchmark_historical_prices["Date"]
                        )
                        benchmark_historical_prices = (
                            benchmark_historical_prices.set_index("Date")
                        )
                    except KeyError:
                        logger.warning(
                            "Benchmark historical prices DataFrame must have a 'Date' index or 'Date' column. Skipping Beta."
                        )

                if isinstance(
                    benchmark_historical_prices.index, pd.DatetimeIndex
                ):  # Check again if conversion succeeded
                    benchmark_historical_prices = (
                        benchmark_historical_prices.sort_index()
                    )
                    benchmark_returns = (
                        benchmark_historical_prices["Close"].pct_change().dropna()
                    )
                    if not benchmark_returns.empty:
                        ...
                        if len(aligned_returns) > 1:
                            ...
                            portfolio_beta = (
                                cov_matrix[0, 1] / benchmark_variance
                                if benchmark_variance != 0
                                else np.nan
                            )
                        else:
                            logger.warning(
                                "Not enough overlapping data for portfolio beta calculation."
                            )
                    else:
                        logger.warning(
                            "Benchmark returns empty, skipping portfolio beta."
                        )
                else:
                    logger.warning(
                        "Benchmark Date index not DatetimeIndex after attempted conversion, skipping Beta."
                    )

            composition_result["risk_metrics"] = {
                "portfolio_volatility": float(portfolio_volatility),
                "portfolio_sharpe_ratio": (
                    float(portfolio_sharpe_ratio)
                    if pd.notna(portfolio_sharpe_ratio)
                    else None
                ),
                "portfolio_max_drawdown": float(portfolio_max_drawdown),
                "portfolio_value_at_risk_95": float(portfolio_var_95),
                "portfolio_beta": (
                    float(portfolio_beta) if pd.notna(portfolio_beta) else None
                ),
            }

            logger.info(
                f"Analyzed portfolio '{tickers}'. Composition and risk metrics calculated."
            )
            return composition_result

        except Exception as e:
            logger.exception(
                f"Error analyzing portfolio: {str(e)}"
    ...
    async def _analyze_performance(
        ...
        try:
            ...
            if len(asset_df["Close"]) < 2:
                return {
                    "error": "Cannot calculate total return with less than 2 price points."
                }
            ...
            num_years = (
                num_days / 365.25
            )  # More precise than using 252 for returns over multiple years
            if num_years > 0:
                annualized_return = (1 + total_return) ** (1 / num_years) - 1
            elif len(asset_returns) > 0:  # If less than a year but has returns
                annualized_return = (
                    asset_returns.mean() * 252
                )  # Fallback to simple annualization for short periods
            else:
                annualized_return = (
                    0.0  # Or total_return if preferred for very short periods
                )
            ...
            if volatility == 0:
                ...
                sharpe_ratio = (
                    annualized_asset_return_for_sharpe - risk_free_rate
                ) / volatility
            ...
            # Monthly Returns
            monthly_returns_series = asset_returns.resample("M").apply(
                lambda x: (1 + x).prod() - 1
            )
            ...
            if (
                ...
                if (
                    ...
                    logger.warning(
                        "Benchmark historical prices DataFrame is missing 'Date' or 'Close' columns. Skipping benchmark metrics."
                    )
                else:
                    benchmark_df = benchmark_historical_prices.copy()
                    benchmark_df["Date"] = pd.to_datetime(benchmark_df["Date"])
                    ...
                    if not benchmark_returns.empty:
                        ...
                        if len(aligned_df) > 1:
                            ...
                            beta = (
                                covariance_with_benchmark / variance_benchmark
                                if variance_benchmark != 0
                                else np.nan
                            )
                            ...
                            annualized_benchmark_return = (
                                aligned_benchmark_returns.mean() * 252
                            )
                            ...
                            logger.warning(
                                "Not enough overlapping data points between asset and benchmark for performance comparison."
                            )
                    else:
                        logger.warning(
                            "Benchmark returns could not be calculated. Skipping benchmark metrics."
                        )

            logger.info(
                f"Analyzed performance. Total Return: {result['total_return']:.2%}"
            )
            return result
    ...
    async def _detect_anomalies(
        ...
        try:
            ...
            if not date_is_index and "Date" not in df.columns:
                return [
                    {
                        "error": "Asset historical prices DataFrame must contain 'Date' column or have a DatetimeIndex."
                    }
                ]
            ...
            if "Close" not in df.columns:
                return [
                    {
                        "error": "Asset historical prices DataFrame must contain 'Close' column."
                    }
                ]
            ...
            if df_returns.empty:
                logger.info(
                    "No returns to analyze for anomalies (e.g., single data point or all NaNs)."
                )
                return []
            ...
            if pd.isna(std_return) or std_return == 0:
                logger.info(
                    "Standard deviation of returns is NaN or zero. Cannot calculate Z-scores for anomalies."
                )
                return []
            ...
            for index, row in anomalies_df.iterrows():
                ...
                anomaly = {
                    ...
                    "z_score": (
                        float(row["Z_Score"]) if pd.notna(row["Z_Score"]) else None
                    ),
                    "direction": (
                        "up"
                        if row["Return"] > 0
                        else ("down" if row["Return"] < 0 else "flat")
                    ),
                    "magnitude": (
    ...
    async def detect_anomalies(
        ...
        str, Any
    ]:  # This method actually returns List[Dict[str,Any]] or List with error dict
    ...
    async def get_earnings_surprises(
        self,
        tickers: List[str],
        api_agent: Any,  # Using Any to avoid direct APIAgent import for now
        ...
        if not api_agent or not hasattr(api_agent, "get_earnings_history"):
            logger.error(
                "APIAgent instance is invalid or missing 'get_earnings_history' method."
            )
        ...
        for ticker in tickers:
            try:
                ...
                if "error" in earnings_history_response:
                    logger.warning(
                        f"Could not fetch earnings history for {ticker}: {earnings_history_response['error']}"
                    )
                    all_surprises[ticker] = [
                        {
                            "error": f"Failed to fetch earnings history: {earnings_history_response['error']}"
                        }
                    ]
                    continue
                ...
                if not earnings_data_list or not isinstance(earnings_data_list, list):
                    logger.info(
                        f"No earnings data or unexpected format for {ticker}: {earnings_data_list}"
                    )
                ...
                date_col_options = [
                    ...
                    "Report Date",
                    "index",
                ]  # Common names after reset_index
                date_col = next(
                    (col for col in date_col_options if col in earnings_df.columns),
                    None,
                )

                if not date_col:
                    logger.warning(
                        f"Earnings date column not found in data for {ticker}. Columns: {earnings_df.columns}"
                    )
                    all_surprises[ticker] = [
                        {"error": "Earnings date column not found."}
                    ]
                    continue
                ...
                if not all(col in earnings_df.columns for col in required_cols):
                    logger.warning(
                        f"Missing required EPS columns in earnings data for {ticker}. Found: {earnings_df.columns}"
                    )
                ...
                try:
                    ...
                    logger.warning(
                        f"Could not parse dates for {ticker}: {date_parse_err}. Dates: {earnings_df[date_col].head()}"
                    )
                    all_surprises[ticker] = [
                        {"error": f"Could not parse dates: {date_parse_err}"}
                    ]
                    continue
                ...
                earnings_df["EPS Estimate"] = pd.to_numeric(
                    earnings_df["EPS Estimate"], errors="coerce"
                )
                earnings_df["Reported EPS"] = pd.to_numeric(
                    earnings_df["Reported EPS"], errors="coerce"
                )

                valid_earnings_df = earnings_df.dropna(
                    subset=["EPS Estimate", "Reported EPS"]
                )

                if valid_earnings_df.empty:
                    logger.info(
                        f"No valid earnings rows with both estimate and reported EPS for {ticker}."
                    )
                    all_surprises[ticker] = []
                    continue

                ticker_surprises = []
                for _, row in valid_earnings_df.head(
                    8
                ).iterrows():  # Take last 8 quarters (after sorting)
                    ...
                    if estimated_eps != 0:
                        ...
                        surprise_percentage = (
                            float("inf") if surprise_eps > 0 else float("-inf")
                        )
                    ...
                    ticker_surprises.append(
                        {
                            ...
                            "surprise_percentage": (
                                round(surprise_percentage, 2)
                                if pd.notna(surprise_percentage)
                                and np.isfinite(surprise_percentage)
                                else "N/A"
                            ),
                        }
                    )
                ...
                logger.info(
                    f"Calculated {len(ticker_surprises)} earnings surprises for {ticker}"
                )

            except Exception as e:
                logger.exception(
                    f"Error calculating earnings surprises for {ticker}: {str(e)}"
                )
                all_surprises[ticker] = [
