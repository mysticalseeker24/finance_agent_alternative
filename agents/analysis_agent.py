...
class AnalysisAgent(BaseAgent):
    ...
    async def process(self, request: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if operation == "calculate_risk":
            ...
            if not data and not tickers:
                return {"error": "Either data or tickers must be provided for risk calculation"}
    ...
    async def _calculate_risk(self, data: Optional[Dict[str, Any]] = None, tickers: Optional[List[str]] = None, period: Optional[str] = "1y") -> Dict[str, Any]:
        ...
        try:
            ...
            volatility = float(data['Return'].std() * np.sqrt(252))  # Annualized volatility
            sharpe_ratio = float(data['Return'].mean() / data['Return'].std() * np.sqrt(252))  # Annualized Sharpe Ratio
            max_drawdown = float((1 - data['Close'] / data['Close'].cummax()).max())  # Maximum drawdown
            var_95 = float(np.percentile(data['Return'].dropna(), 5))  # 95% Value at Risk
    ...
    async def _analyze_portfolio(self, portfolio: List[Dict[str, Any]], market_data: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            weights = [item.get("weight", 0) for item in portfolio if item.get("ticker")]
            ...
            diversification_status = "Well Diversified" if concentration < 0.15 else \
                                     "Moderately Diversified" if concentration < 0.25 else \
                                     "Concentrated"
    ...
    async def _analyze_performance(self, data: Dict[str, Any], benchmark: Optional[Dict[str, Any]] = None, period: Optional[str] = "1y") -> Dict[str, Any]:
        ...
        try:
            ...
            annualized_return = float((1 + total_return) ** (252 / len(data)) - 1) if len(data) > 1 else 0
            volatility = float(data['Return'].std() * np.sqrt(252))
            sharpe_ratio = float(data['Return'].mean() / data['Return'].std() * np.sqrt(252)) if volatility > 0 else 0
            ...
            if 'Date' in data.columns:
                ...
                monthly_returns = data.groupby(['Year', 'Month'])['Return'].sum().reset_index()
            ...
            if benchmark:
                ...
                if 'Close' in benchmark.columns:
                    ...
                    benchmark_total_return = float(benchmark['Close'].iloc[-1] / benchmark['Close'].iloc[0] - 1)
                    benchmark_annualized_return = float((1 + benchmark_total_return) ** (252 / len(benchmark)) - 1) if len(benchmark) > 1 else 0
    ...
    async def _detect_anomalies(self, data: Dict[str, Any], threshold: float = 2.0) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            data['Z_Score'] = (data['Return'] - mean_return) / std_return if std_return > 0 else 0
            ...
            for _, row in anomalies.iterrows():
                anomaly = {
                    ...
                    "magnitude": "extreme" if abs(row['Z_Score']) > 3.0 else "significant",
                }
    ...
    async def calculate_risk(self, data: Optional[Dict[str, Any]] = None, tickers: Optional[List[str]] = None, period: Optional[str] = "1y") -> Dict[str, Any]:
        """Calculate risk metrics for stocks or portfolios.
    ...
    async def analyze_portfolio(self, portfolio: List[Dict[str, Any]], market_data: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """Analyze a portfolio's composition and risk.
    ...
    async def analyze_performance(self, data: Dict[str, Any], benchmark: Optional[Dict[str, Any]] = None, period: Optional[str] = "1y") -> Dict[str, Any]:
        """Analyze performance of a stock or portfolio.
    ...
    async def detect_anomalies(self, data: Dict[str, Any], threshold: float = 2.0) -> Dict[str, Any]:
        """Detect anomalies in financial data.
