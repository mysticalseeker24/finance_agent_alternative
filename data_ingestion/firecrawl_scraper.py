...
class FirecrawlScraper:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        if not self.api_key:
            logger.error(
                "Firecrawl API key not configured. FirecrawlScraper will not work."
            )
        else:
            try:
    ...
    async def scrape_url(
        ...
        if not self.app:
            return {
                "success": False,
                "url": url_to_scrape,
                "error": "FirecrawlApp not initialized due to missing API key or initialization failure.",
            }

        current_page_options = (
            page_options
            if page_options is not None
            else {"onlyMainContent": True, "includeHtml": False}
        )

        logger.info(
            f"Requesting Firecrawl SDK scrape for URL: {url_to_scrape} with page_options: {current_page_options}, extractor_options: {extractor_options}"
        )
        ...
        try:
            ...
            sdk_response = await asyncio.to_thread(
                ...
                * 1000,  # Convert seconds to milliseconds for SDK
            )
            ...
            chosen_content = sdk_response.get("markdown") or sdk_response.get(
                "content", ""
            )
        ...
        # The FirecrawlApp might raise its own specific exceptions.
        ...
        ):  # This catches timeout for asyncio.to_thread itself
            logger.error(
                f"asyncio.to_thread call timed out scraping {url_to_scrape} after {timeout_seconds} seconds."
            )
            return {
                "success": False,
                "url": url_to_scrape,
            ...
            logger.error(
                f"Firecrawl SDK error scraping {url_to_scrape}: {type(e).__name__} - {str(e)}"
            )
    ...
    async def scrape_financial_news(
        ...
        for source_name, url in sources_to_scrape_map.items():
            ...
            scrape_result = await self.scrape_url(
                url, page_options=page_options_for_news_landing
            )
            ...
            if scrape_result.get("success"):
                results.append(
                    {
                        ...
                        "content": scrape_result.get(
                            "content"
                        ),  # This will be markdown or text of the main page
                        "metadata": scrape_result.get("metadata", {}),
                        "raw_html": (
                            scrape_result.get("raw_data", {}).get("html")
                            if page_options_for_news_landing.get("includeHtml")
                            else None
                        ),
                    }
                )
                ...
                logger.warning(
                    f"Failed to scrape news source: {source_name} from {url}. Error: {scrape_result.get('error')}"
                )
                results.append(
                    {
                        "source_name": source_name,
    ...
    async def scrape_earnings_report(self, ticker: str) -> Dict[str, Any]:
        ...
        logger.info(
            f"Scraping earnings report page for ticker: {ticker} from {earnings_page_url}"
        )
        ...
        scrape_result = await self.scrape_url(
            earnings_page_url, page_options=page_options_for_earnings
        )
        ...
        if scrape_result.get("success"):
            ...
            logger.warning(
                f"Failed to scrape earnings report for {ticker} from {earnings_page_url}. Error: {scrape_result.get('error')}"
            )
            return {
                "ticker": ticker,
                "url": earnings_page_url,
    ...
    async def crawl_website(
        self,
        url_to_crawl: str,
        crawler_options: Optional[
            Dict
        ] = None,  # These are the 'params' for SDK's crawl_url
        ...
        logger.info(
            f"Requesting Firecrawl SDK crawl for URL: {url_to_crawl} with crawler_options: {crawler_options}"
        )
        ...
        try:
            ...
            if crawl_results_sdk:  # Ensure it's not None or empty
                for item_data in crawl_results_sdk:
                    ...
                    chosen_content = item_data.get("markdown") or item_data.get(
                        "content", ""
                    )
            ...
            logger.info(
                f"Successfully crawled URL via SDK: {url_to_crawl}, found {len(processed_results)} pages."
            )
            return processed_results

        except asyncio.TimeoutError:
            logger.error(
                f"asyncio.to_thread call timed out crawling {url_to_crawl} after {timeout_seconds} seconds."
            )
            return [
                {
                    "success": False,
                    ...
                    "details": f"Crawl operation timed out after {timeout_seconds} seconds.",
                }
            ]
        except Exception as e:
            logger.error(
                f"Firecrawl SDK error crawling {url_to_crawl}: {type(e).__name__} - {str(e)}"
            )
            details = str(e)
            ...
            return [
                {
                    "success": False,
                    "url": url_to_crawl,
                    "error": f"Firecrawl SDK Crawl Error: {type(e).__name__}",
                    "details": details,
                }
            ]
