...
class DocumentLoader:
    ...
    def extract_text_from_pdf(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
                            ...
                            if table and len(table) > 1:  # Ensure table has headers and data
                                ...
                                for row in rows:
                                    ...
                                    for k, header in enumerate(headers):
                                        if k < len(row) and header:  # Ensure header exists
            ...
            logger.error(f"Error extracting text from {file_path} using PDFPlumber: {str(e)}")
            # Fallback to PyPDF2
    ...
    def extract_text_from_pdf_simple(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Error extracting text from {file_path} using PyPDF2: {str(e)}")
            return {"error": str(e), "source": file_path}
    
    def extract_text_from_bytes(self, file_bytes: Union[bytes, BinaryIO], filename: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Error extracting text from {filename} using PDFPlumber: {str(e)}")
            # Fallback to PyPDF2
            ...
            try:
                ...
                logger.error(f"Error extracting text from {filename} using PyPDF2: {str(e2)}")
    ...
    def extract_structured_data(self, file_path: str) -> Dict[str, Any]:
        ...
        financial_terms = {
            ...
            "income_statement": ["income statement", "revenue", "earnings", "profit", "loss"],
        ...
        if not structured_data["financial_data"]:
            ...
            all_text = " ".join(page["text"].lower() for page in extraction_result["pages"])
