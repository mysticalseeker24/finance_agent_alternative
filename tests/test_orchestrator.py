...
class TestOrchestrator(unittest.TestCase):
    ...
    async def test_process_stock_query(self, mock_generate_response, mock_get_stock_info):
    ...
    async def test_generate_market_brief(self, mock_generate_narrative, 
                                        mock_get_portfolio_data, mock_get_market_summary):
        ...
        mock_get_market_summary.return_value = {
            "data": {
                ...
                "^DJI": {"name": "Dow Jones", "price": 38456.78, "change_percent": -0.09}
            }
        }
    ...
    @mock.patch.object(APIAgent, 'get_stock_info')
    @mock.patch.object(LanguageAgent, 'generate_response')
    async def test_complex_query_with_retrieval(self, mock_generate_response, 
                                              mock_get_stock_info, mock_search_content):
